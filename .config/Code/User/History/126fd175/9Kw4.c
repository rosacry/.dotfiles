...
int read_distance_data(FILE *in) {
    ...
    while ((c = fgetc(in)) != EOF) {                     // begin parsing text
        if (!remname) {                                  // removing column name
            ...
            nodes[rows].name = node_names[rows];  // do i need to do this?
            for (int i = 0; i < cname; i++) {
                *(*(node_names + rows) + i) = *(input_buffer + cname);
            }
            remname = 1;
            ...
            *(input_buffer + val_in) = '\0';  // null terminating character to end the character being parsed
            if (rows == 0) {                  // letters (taxons)
                ...
                // do this next
                nodes[rows].name = node_names[rows];  // set the row (taxon) name equal to the node
                for (int i = 0; i < val_in; i++) {
                    *(*(node_names + rows) + i) = *(input_buffer);
                }
                *(*(node_names + rows) + val_in) = '\0';
